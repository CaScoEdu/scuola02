const FILE_SCUOLA = "./scuola02.json";
// Identificatore del contenitore HTML per le classi
const SEZIONE_CLASSI = document.getElementById("classi");

async function loadAndDisplayScuola() {
  try {
    // 1. Richiesta di rete
    const response = await fetch(FILE_SCUOLA);
    ...
    scuolaData.forEach((classe) => {
      ...
      titoloClasse.textContent = `${classe.anno}${classe.sezione} ${classe.indirizzo} - ${classe.numeroAlunni} alunni`;
      ...
      classe.materie.forEach((materia) => {
        ...
        const statoObbligatoria = materia.obbligatoria
          ? "obbligatoria"
          : "non obbligatoria";
      ...
      SEZIONE_CLASSI.appendChild(sezioneClasse);
    });
  } catch (errore) {
    console.error("Errore durante l'operazione:", errore);
    SEZIONE_CLASSI.textContent =
      "Impossibile caricare o visualizzare i dati scolastici.";
  }
}
